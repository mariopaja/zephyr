...
__subsystem struct i2s_driver_api {
	...
	enum i2s_state (*get_state)(const struct device *dev);
};
/**
 * @endcond
 */
 ...
 */
__syscall enum i2s_state i2s_get_state(const struct device *dev);

static inline enum i2s_state z_impl_i2s_get_state(const struct device *dev)
{
	return DEVICE_API_GET(i2s, dev)->get_state(dev);
}
