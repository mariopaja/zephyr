...
#define DT_DRV_COMPAT nvu_nv3030b
...
LOG_MODULE_REGISTER(display_nv3030b);
...
struct st7789v_config {
	...
	uint8_t lcm;
	bool inversion_on;
	uint8_t dvdd_param[2];
	uint8_t bias_param[3];
	uint8_t vgl_param[4];
	uint8_t vgh_param[1];
	uint8_t vsp_param[3];
	uint8_t vsn_param[3];
	uint8_t pump_ck_param[2];
	uint8_t gam_ref1_param[4];
	uint8_t frate_param[3];
	uint8_t dpolctrl;
	uint8_t porch_param[4];
	uint8_t dfunc_param[5];
	uint8_t gam_p3_param[6];
	uint8_t gam_n3_param[6];
	uint8_t gam_p2_param[2];
	uint8_t gam_n2_param[2];
	uint8_t gam_p1_param[8];
	uint8_t gam_n1_param[8];
	uint8_t src1_param[2];
	uint8_t src2_param[6];
	uint8_t src3_param[3];
	uint8_t gate_timing;
	uint8_t tear_param[3];
	uint8_t iface_param[4];
...
static int st7789v_lcd_init(const struct device *dev)
{
	struct st7789v_data *data = dev->data;
	const struct st7789v_config *config = dev->config;
	uint8_t priv_en[2] = {0x06, 0x08};
	uint8_t priv_dis[2] = {0xFA, 0xFC};
	uint8_t tmp;
	int ret;
	...
	/* Enable Write/Read on Priv Registers */
	ret = st7789v_transmit(dev, NV3030B_CMD_PRIV, (uint8_t *)priv_en, sizeof(priv_en));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_DVDD, (uint8_t *)config->dvdd_param,
			       sizeof(config->dvdd_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_BIAS, (uint8_t *)config->bias_param,
			       sizeof(config->bias_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_VGL, (uint8_t *)config->vgl_param,
			       sizeof(config->vgl_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_VGH, (uint8_t *)config->vgh_param,
			       sizeof(config->vgh_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_VSP, (uint8_t *)config->vsp_param,
			       sizeof(config->vsp_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_VSN, (uint8_t *)config->vsn_param,
			       sizeof(config->vsn_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_PUMP_CK, (uint8_t *)config->pump_ck_param,
			       sizeof(config->pump_ck_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAM_REF1, (uint8_t *)config->gam_ref1_param,
			       sizeof(config->gam_ref1_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_FRATE, (uint8_t *)config->frate_param,
			       sizeof(config->frate_param));
	if (ret < 0) {
		return ret;
	}

	tmp = config->dpolctrl;
	ret = st7789v_transmit(dev, NV3030V_CMD_DPOLCTRL, &tmp, 1);
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030V_CMD_PORCTRL, (uint8_t *)config->porch_param,
			       sizeof(config->porch_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_DFUNC, (uint8_t *)config->dfunc_param,
			       sizeof(config->dfunc_param));
	if (ret < 0) {
		return ret;
	}

	tmp = config->gamma;
	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_EN, &tmp, 1);
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_P3, (uint8_t *)config->gam_p3_param,
			       sizeof(config->gam_p3_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_N3, (uint8_t *)config->gam_n3_param,
			       sizeof(config->gam_n3_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_P2, (uint8_t *)config->gam_p2_param,
			       sizeof(config->gam_p2_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_N2, (uint8_t *)config->gam_n2_param,
			       sizeof(config->gam_n2_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_P1, (uint8_t *)config->gam_p1_param,
			       sizeof(config->gam_p1_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_GAMMA_N1, (uint8_t *)config->gam_n1_param,
			       sizeof(config->gam_n1_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_SRCCTRL1, (uint8_t *)config->src1_param,
			       sizeof(config->src1_param));
	if (ret < 0) {
		return ret;
	}


	ret = st7789v_transmit(dev, NV3030B_CMD_SRCCTRL2, (uint8_t *)config->src2_param,
			       sizeof(config->src2_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_SRCCTRL3, (uint8_t *)config->src3_param,
			       sizeof(config->src3_param));
	if (ret < 0) {
		return ret;
	}

	tmp = config->gate_timing;
	ret = st7789v_transmit(dev, NV3030B_GATE_TIMING, &tmp, 1);
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_TEAR_EFF, (uint8_t *)config->tear_param,
			       sizeof(config->tear_param));
	if (ret < 0) {
		return ret;
	}

	ret = st7789v_transmit(dev, NV3030B_CMD_IFACECTRL, (uint8_t *)config->iface_param,
			       sizeof(config->iface_param));
	if (ret < 0) {
		return ret;
	}

	/* Disale Write/Read on Priv Registers */
	ret = st7789v_transmit(dev, NV3030B_CMD_PRIV, (uint8_t *)priv_dis, sizeof(priv_dis));
	if (ret < 0) {
		return ret;
	}
	...
	tmp = 0;
	ret = st7789v_transmit(dev, ST7789V_CMD_INV_ON, &tmp, 1);
	if (ret < 0) {
		return ret;
	}
	...
	tmp = 0x0;
	ret = st7789v_transmit(dev, NV3030B_CMD_TEAR_EFF_LINEON, &tmp, 1);
	if (ret < 0) {
		return ret;
	}
...
#define ST7789V_INIT(inst)								\
	static const struct st7789v_config st7789v_config_ ## inst = {			\
		...
		.dvdd_param = DT_INST_PROP(inst, dvdd_param),			\
		.bias_param = DT_INST_PROP(inst, bias_param),			\
		.vgl_param = DT_INST_PROP(inst, vgl_param),			\
		.vgh_param = DT_INST_PROP(inst, vgh_param),			\
		.vsp_param = DT_INST_PROP(inst, vsp_param),			\
		.vsn_param = DT_INST_PROP(inst, vsn_param),			\
		.pump_ck_param = DT_INST_PROP(inst, pump_ck_param),			\
		.gam_ref1_param = DT_INST_PROP(inst, gam_ref1_param),			\
		.frate_param = DT_INST_PROP(inst, frate_param),			\
		.dpolctrl = DT_INST_PROP(inst, dpolctrl),					\
		.dfunc_param = DT_INST_PROP(inst, dfunc_param),					\
		.gam_p3_param = DT_INST_PROP(inst, gam_p3_param),					\
		.gam_n3_param = DT_INST_PROP(inst, gam_n3_param),					\
		.gam_p2_param = DT_INST_PROP(inst, gam_p2_param),					\
		.gam_n2_param = DT_INST_PROP(inst, gam_n2_param),					\
		.gam_p1_param = DT_INST_PROP(inst, gam_p1_param),					\
		.gam_n1_param = DT_INST_PROP(inst, gam_n1_param),					\
		.src1_param = DT_INST_PROP(inst, src1_param),					\
		.src2_param = DT_INST_PROP(inst, src2_param),					\
		.src3_param = DT_INST_PROP(inst, src3_param),					\
		.gate_timing = DT_INST_PROP(inst, gate_timing),					\
		.tear_param = DT_INST_PROP(inst, tear_param),					\
		.iface_param = DT_INST_PROP(inst, iface_param),					\
