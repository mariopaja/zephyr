...
*/
	...
	lan9250_write_phy_reg(dev, LAN9250_PHY_BASIC_CONTROL, LAN9250_PHY_BASIC_CONTROL_PHY_AN |LAN9250_PHY_BASIC_CONTROL_PHY_SPEED_SEL_LSB| LAN9250_PHY_BASIC_CONTROL_PHY_DUPLEX);
...
static void lan9250_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		LOG_ERR("Interrupt status: %08x", isr);
		     ...
		     if ((isr & 0x01000000) != 0)
        {
          LOG_ERR("RX stop");
		      ...
		      if ((isr & 0x00800000) != 0)
        {
          LOG_ERR("RX dropped frame count halfway");
        }
		      ...
		      if ((isr & 0x00080000) != 0)
        {
          LOG_ERR("GP-timer");
		      ...
		      if ((isr & 0x00020000) != 0)
        {
          LOG_ERR("Power management");
        }

		      if ((isr & 0x00010000) != 0)
        {
          LOG_ERR("TX FIFO status overflow");
        }

		 if ((isr & 0x00008000) != 0)
        {
          LOG_ERR("RX watchdog timeout");
        }
		      ...
		      if ((isr & 0x00002000) != 0)
        {
          LOG_ERR("TX error");
		      ...
		      if ((isr & 0x00001000) != 0)
        {
          LOG_ERR("ISR_GPIO event");
        }

		     if ((isr & 0x00000400) != 0)
        {
          LOG_ERR("TX data FIFO overrun");
        }

		      if ((isr & 0x00000200) != 0)
        {
          LOG_ERR("TX data FIFO available");

          //lan9250_txavailable_isr(priv);
		  	lan9250_write_sys_reg(context->dev, LAN9250_FIFO_INT,
			      LAN9250_FIFO_INT_TX_DATA_AVAILABLE_LEVEL |
				      LAN9250_FIFO_INT_TX_STATUS_LEVEL);
        }
		      ...
		      if ((isr & 0x00000100) != 0)
        {
          LOG_ERR("TX status FIFO full");
        }

		      if ((isr & 0x00000080) != 0)
        {
          LOG_ERR("TX status FIFO level");
        }

		      if ((isr & 0x00000040) != 0)
        {
          LOG_ERR("RX dropped frame");
        }

		      if ((isr & 0x00000010) != 0)
        {
          LOG_ERR("RX status FIFO full");
        }

		if ((isr & LAN9250_INT_STS_RSFL) != 0) {
			LOG_ERR("RX status FIFO level");
			lan9250_rx(context->dev);
		}
		lan9250_write_sys_reg(context->dev, LAN9250_INT_STS, 0xFFFFFFFF);
